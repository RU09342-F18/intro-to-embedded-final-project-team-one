...
void loop()
{
	...
	if (fps.IsPressFinger())
	{
		...
		int id = fps.Identify1_N();
	     ...
	     /*Note:  GT-521F52 can hold 1000 fingerprint templates
			 ...
			 model you are using */
     if (id == 6){
      Serial.print("Welcome Eric ");
      digitalWrite(13, HIGH); // Set pin 13 High
      delay(1000);            // Delay
      digitalWrite(13, LOW);  // Set pin 13 Low
     }
     //sends pulse to pin 13
      
   else if(id == 9){
    Serial.print("Welcome Kyle");
    digitalWrite(12, HIGH); // Set pin 12 High
      delay(1000);            // Delay
      digitalWrite(12, LOW);  // Set pin 12 Low
   } //sends pulse to pin 12
else if (id <20) //<- change id value depending model you are using
